# 🏛️ المرجع المعماري الشامل والنهائي (النسخة الثانية المحسنة — V2)
...
10. [سيناريو حركة وتسجيل المرضى بين المستشفيات](#7-سيناريو-حركة-وتسجيل-المرضى-بين-المستشفيات)
11. [التواصل بين الموديولات — Cross-Module Communication](#11-التواصل-بين-الموديولات)
12. [هيكل الـ Solution وخطة التنفيذ](#12-هيكل-الـ-solution-وخطة-التنفيذ)
...
erDiagram
    ...
    CentralPatients {
        Guid     Id            PK "Global Patient GUID"
        string   NationalId    UK "الرقم القومي / جواز السفر"
        string   FullName
        datetime BirthDate
        string   Gender
        string   Phone
    ...
    PatientTenantLinks {
        Guid     Id            PK
        Guid     PatientId     FK "CentralPatients.Id"
        Guid     TenantId      FK "Tenants.Id"
        string   LocalMRN      "رقم الملف الطبي المحلي المخصص"
        datetime LinkedAt
    }
    ...
    CentralPatients ||--o{ PatientTenantLinks   : "مسجل في مستشفيات"
    Tenants         ||--o{ PatientTenantLinks   : "يسجل المرضى"
```
...
private async Task<LoginResponseDto> BuildHospitalUserResponse(CentralUser user, List<Claim> claims)
{
    ...
    // 💡 تحديث معمارية V2 لمنع تضخم الـ JWT وحمايتها من أخطاء HTTP 431:
    // لا نقوم بوضع كل المستشفيات وأدوارها داخل توكن الـ JWT الأساسي لتجنب تجاوز حجم Headers.
    // التوكن يحمل فقط الهوية الأساسية، بينما يتم التحقق من الصلاحيات وتفاصيل الدور
    // ديناميكياً في الـ Middleware مع كل طلب بناءً على الـ Header X-Tenant-Key،
    // مع استخدام Cache فائق السرعة لضمان أفضل أداء وعملية سحب فوري للصلاحيات.
...
flowchart LR
    subgraph CDB["🗄️ Central DB"]
        ...
        PTL["PatientTenantLinks\nPatientId: ❸ GUID-PATIENT-XYZ\nTenantId: ❷ GUID-TENANT-ABC\nLocalMRN: HOSP-A-00023"]
    end

    subgraph TDB["🏥 Tenant DB"]
        TU["TenantUsers (Shadow)\nId: ❶ GUID-USER-123\n← نفس الـ GUID!"]
        HP["HospitalProfiles\nTenantId: ❷ GUID-TENANT-ABC\n← نفس الـ GUID!"]
        LPR["LocalPatientRecords\nPatientId: ❸ GUID-PATIENT-XYZ\n← نفس الـ GUID!\nMRN: HOSP-A-00023\n← الترقيم المحلي المخصص!"]
        PT["Patients (Clinical Shadow)\nId: ❸ GUID-PATIENT-XYZ\n← نفس الـ GUID!"]
    end

    CU -->|"UserAssignedToTenantEvent"| TU
    TN -->|"TenantProvisionedEvent"| HP
    CP -->|"PatientRegisteredToTenantEvent"| LPR
    LPR --> PT
    TU --> PT
```
...
### جدول `LocalPatientRecords` (Shadow Patient Record inside Tenant DB)

يحتوي على الربط بين الـ Patient GUID الموحد وبين ترقيم المستشفى المحلي المستقل (MRN).

| Column | Type | Description |
|:---|:---|:---|
| `PatientId` | `Guid` PK | **نفس** `CentralPatients.Id` من قاعدة البيانات المركزية |
| `TenantId` | `Guid` FK | معرف المستشفى الحالي (TenantId) |
| `MRN` | `NVarChar(50)` UK | **رقم الملف الطبي المحلي (Medical Record Number)** الخاص بهذه المستشفى |
| `RegisteredAt` | `DateTime` | تاريخ التسجيل في هذه المستشفى |
| `IsActive` | `Bit` | حالة الملف الطبي في هذا المستشفى |
| `SyncedAt` | `DateTime` | تاريخ آخر مزامنة |
...
## 7. سيناريو حركة وتسجيل المرضى بين المستشفيات (Centralized Patient Identity)

بناءً على طلب العميل، تم تصميم دورة حياة المريض لتكون **موحدة الهوية (Global GUID) ولامركزية الترقيم المحلي (Local MRN)**. يتيح ذلك للمريض التنقل بين مستشفيات المنصة مع الحفاظ على ملف طبي موحد دون فقدان استقلالية كل مستشفى في تخصيص أرقام ملفاتها الطبية.

### 7.1 سيناريوهات تسجيل ومتابعة المرضى

#### السيناريو A: تسجيل مريض جديد تماماً في المنظومة لأول مرة (New Global Patient)

```mermaid
sequenceDiagram
    actor Receptionist as 👤 موظف الاستقبال (مستشفى أ)
    participant Central as 🗄️ Central DB
    participant TenantA as 🏥 Tenant DB (مستشفى أ)
    participant Bus as 📮 Event Bus

    Receptionist->>Central: بحث بالرقم القومي (NationalId: 29801...)
    Central-->>Receptionist: غير موجود ❌ (مريض جديد تماماً)
    Receptionist->>Central: تسجيل بيانات الهوية الأساسية (الاسم، تاريخ الميلاد، الرقم القومي)
    Central->>Central: توليد GUID موحد (PatientId = GUID-XYZ)
    Central-->>Receptionist: تم الحفظ بنجاح

    Receptionist->>TenantA: طلب فتح ملف طبي محلي في مستشفى أ
    TenantA->>TenantA: توليد رقم ملف طبي محلي متسلسل (Local MRN = HOSP-A-00001)
    TenantA->>TenantA: حفظ في جدول LocalPatientRecords (GUID-XYZ, MRN: HOSP-A-00001)
    
    TenantA->>Central: تسجيل الربط في PatientTenantLinks
    
    TenantA->>Bus: إرسال حدث PatientCreatedEvent { PatientId=GUID-XYZ, MRN=HOSP-A-00001 }
    Note over Bus: موديولات المستشفى أ (المختبر، الصيدلية) تستقبل الحدث وتنشئ Shadow Patient محلياً بنفس الـ GUID
```

#### السيناريو B: مريض مسجل مسبقاً يزور مستشفى جديدة في المنصة (Existing Central Patient, New Hospital)

```mermaid
sequenceDiagram
    actor Receptionist as 👤 موظف الاستقبال (مستشفى ب)
    participant Central as 🗄️ Central DB
    participant TenantB as 🏥 Tenant DB (مستشفى ب)
    participant Bus as 📮 Event Bus

    Receptionist->>Central: بحث بالرقم القومي (NationalId: 29801...)
    Central-->>Receptionist: تم العثور على المريض! ✅ (GUID-XYZ, الاسم: علي محمد)
    Note over Receptionist: تظهر قائمة بالمستشفيات التي زارها المريض مسبقاً (مستشفى أ بملف HOSP-A-00001)

    Receptionist->>TenantB: فتح ملف طبي للمريض علي محمد (GUID-XYZ) في مستشفى ب
    TenantB->>TenantB: توليد رقم ملف طبي محلي متسلسل خاص بمستشفى ب (Local MRN = HOSP-B-99823)
    TenantB->>TenantB: حفظ في جدول LocalPatientRecords (GUID-XYZ, MRN: HOSP-B-99823)
    
    TenantB->>Central: تسجيل الربط في PatientTenantLinks
    
    TenantB->>Bus: إرسال حدث PatientAssignedToTenantEvent { PatientId=GUID-XYZ, MRN=HOSP-B-99823 }
    Note over Bus: موديولات المستشفى ب تستمع للحدث وتنشئ Shadow Patient محلياً بملف HOSP-B-99823
```

### 7.2 فوائد هذا التصميم
1. **تجنب تكرار البيانات:** الهوية الحقيقية للمريض واحدة بالمنظومة بالكامل، مما يمنع إنشاء سجلات مكررة لنفس المريض.
2. **استقلالية الترقيم:** تحتفظ كل مستشفى بحريتها الكاملة في نظام الترقيم (MRN Prefix / Sequential numbering / custom logic).
3. **الملف الطبي الموحد (Shared EMR):** يسهل مستقبلاً الاستعلام عن التاريخ المرضي للمريض بالكامل عبر الـ GUID المشترك من كافة المستشفيات في حال الطوارئ أو بموافقة المريض.

### 7.3 آلية تحديث وتزامن بيانات المرضى (Patient Update & Lifecycle Architecture)

لحل مشكلة تحديث البيانات (مثل الاسم، أو الهاتف) وانعكاسها بشكل متسق وصحيح بين المستشفيات المشتركة وبقية الموديولات دون التسبب في تداخل البيانات أو بطء الاستعلامات، تعتمد المنظومة على الآلية التالية:

#### 1. تقسيم البيانات حسب طبيعتها (Data Classification)
* **البيانات العالمية (Global Data):** الاسم الكامل، الرقم القومي، تاريخ الميلاد، الجنس، فصيلة الدم، وهاتف المريض الأساسي. تُحفظ في الـ Central DB (جدول `CentralPatients`).
* **البيانات المحلية والسريرية (Local & Clinical Data):** رقم الملف الطبي المحلي (`MRN`)، الحساسيات، الزيارات، الأدوية، والتحاليل. تُحفظ في قاعدة بيانات المستشفى المعنية (`Tenant DB`).

#### 2. مخطط حركة التحديث والتزامن (Update Sequence Flow)

عند قيام موظف استقبال في **مستشفى السلام (Tenant A)** بتحديث اسم المريض "علي محمد" (المسجل أيضاً في **مستشفى القناة Tenant B**) إلى "علي محمد السيد":

```mermaid
sequenceDiagram
    actor User as 👤 موظف الاستقبال (مستشفى أ)
    participant PatAPI as 💻 Patient Module (Host A)
    participant Central as 🗄️ Central DB (CentralPatients)
    participant Bus as 📮 Event Bus (MassTransit)
    participant TenantA as 🏥 Tenant DB (مستشفى أ)
    participant TenantB as 🏥 Tenant DB (مستشفى ب)

    User->>PatAPI: تحديث الاسم إلى "علي محمد السيد" والهاتف
    
    # 1. تحديث السنترال أولاً
    PatAPI->>Central: UPDATE CentralPatients SET FullName = 'علي محمد السيد' WHERE Id = GUID-XYZ
    
    # 2. تحديث النسخة المحلية في مستشفى أ
    PatAPI->>TenantA: UPDATE LocalPatientRecords SET FullName = 'علي محمد السيد' WHERE PatientId = GUID-XYZ
    
    # 3. إطلاق حدث التحديث للمنظومة بالكامل
    PatAPI->>Bus: Publish(PatientUpdatedEvent { PatientId: GUID-XYZ, FullName: 'علي محمد السيد', Phone: '...' })
    
    # 4. التزامن اللامتزامن في الخلفية
    par مستشفى أ (تحديث موديولاتها الداخلية)
        Bus->>TenantA: (Lab / Pharmacy Consumers)
        TenantA->>TenantA: UPDATE LabPatients & PharmacyPatients SET FullName = 'علي محمد السيد' WHERE Id = GUID-XYZ
    and مستشفى ب (المستشفيات الأخرى)
        Bus->>TenantB: (Patient Tenant B Consumer)
        Note over TenantB: يتحقق: هل المريض مسجل لدينا؟<br/>(نعم، موجود بملف QANAL-505)
        TenantB->>TenantB: UPDATE LocalPatientRecords SET FullName = 'علي محمد السيد' WHERE PatientId = GUID-XYZ
        TenantB->>Bus: (Lab / Pharmacy Consumers لـ Tenant B)
        TenantB->>TenantB: تحديث جداول الظل الداخلية لـ Tenant B
    end
```

#### 3. التصفية الذكية للأحداث (Selective Event Consumption)
* المستشفيات الأخرى التي **لم يزرها المريض أبداً** (ولا تملك سجلاً له في `LocalPatientRecords`) تستقبل الحدث عبر الـ Consumer الخاص بها، وتقوم بعمل استعلام محلي سريع جداً:
  `SELECT 1 FROM LocalPatientRecords WHERE PatientId = 'GUID-XYZ'`
* في حال عدم وجود المريض، يتم **تجاهل الحدث تماماً وإلغاؤه** (Discard).
* يضمن هذا بقاء حجم قواعد بيانات المستشفيات خفيفاً ولا يحتوي إلا على سجلات المرضى الفعليين للمستشفى.

#### 4. تزامن الموديولات داخل المستشفى (Module Shadow Sync)
* للحفاظ على مبدأ عزل البيانات واستقلالية قواعد البيانات لكل موديول (Lab, Pharmacy, EMR):
* يمتلك كل موديول جدول ظل محلي (مثل `LabPatients`) لتجنب الـ Cross-Module Joins.
* عند استقبال حدث التحديث، يقوم الـ Consumer الخاص بالموديول بتحديث جدول الظل فوراً محلياً لضمان سرعة القراءة وعرض البيانات المحدثة لحظياً على الشاشات والتقارير.

## 8. تسجيل قاعدة بيانات جديدة وإضافة مستشفى
...
public class TenantResolutionMiddleware
{
    ...
    public TenantResolutionMiddleware(RequestDelegate next, IMemoryCache cache, IEncryptionService encryptor)
        => (_next, _cache, _encryptor) = (next, cache, encryptor);
                                   ...
                                   CentralDbContext centralDb)
    {
        ...
        var isSystemAdmin = ctx.User.HasClaim("user_type", UserType.SystemAdmin.ToString());

        // 2. التحقق وتأكيد الصلاحية ديناميكياً (تجنباً لتضخم JWT وحفاظاً على الأمان)
        var roleCacheKey = $"user_role_{userId}_{tenantKey}";
        if (!_cache.TryGetValue(roleCacheKey, out (string Role, string Department)? userAssignedRole))
        {
            if (!isSystemAdmin)
            {
                var userRole = await centralDb.UserTenantRoles
                    .Include(utr => utr.Tenant)
                    .Where(utr => utr.UserId == userId && utr.Tenant.TenantKey == tenantKey.ToString() && utr.IsActive)
                    .Where(utr => utr.ExpiresAt == null || utr.ExpiresAt > DateTime.UtcNow)
                    .FirstOrDefaultAsync();
                ...
                userAssignedRole = (userRole.RoleName, userRole.Department);
            }
            else
            {
                userAssignedRole = ("SystemAdmin", "IT");
            }

            _cache.Set(roleCacheKey, userAssignedRole, TimeSpan.FromMinutes(5));
        }
        ...
        // 4. تعيين الدور والقسم ديناميكياً للطلب الحالي
        tenantContext = tenantContext with 
        { 
            UserRole = userAssignedRole.Value.Role, 
            Department = userAssignedRole.Value.Department 
        };

        // 5. تسجيل في الـ Accessor — متاح لكل DbContext في هذا الـ Request
    ...
    where TContext : DbContext
{
    ...
    // 💡 تحديث معمارية V2 لمنع ثغرة الـ Model Cache في EF Core:
    // نحدد خواص ديناميكية تقرأ من الـ Context في كل Query بدلاً من Constant ثابت
    public Guid CurrentTenantId => TenantAccessor.TenantContext?.TenantId ?? Guid.Empty;
    public bool IsSoleTenant => TenantAccessor.TenantContext?.Mode == TenantMode.SoleTenant;
    ...
    // ─── 2. الفلتر التلقائي (يُطبَّق ديناميكياً لحل مشكلة الكاش) ──────────────
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        foreach (var entityType in modelBuilder.Model.GetEntityTypes())
        {
            ...
            {
                // بناء التعبير ديناميكياً ليقرأ من Property الخاص بالـ Context نفسه في كل Query
                // المعادلة تعادل: e => e.TenantId == this.CurrentTenantId
                ...
                var contextParam = Expression.Constant(this);
                var currentTenantIdProp = Expression.Property(contextParam, nameof(CurrentTenantId));
                
                // e.TenantId == context.CurrentTenantId
                var equals = Expression.Equal(tenantIdProp, currentTenantIdProp);
...
## 10. الـ Deploy المستقل لكل موديول
...
## 12. هيكل الـ Solution وخطة التنفيذ
